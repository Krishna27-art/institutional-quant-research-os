...
class HybridHMMCPD:
    ...
    def extract_features(self, data: pd.DataFrame) -> np.ndarray:
        ...
        # Skewness (20-day)
        skew = returns.rolling(20).skew().iloc[-1]
    ...
    def fit(self, data: pd.DataFrame, window_days: int = 252) -> None:
        ...
        feature_matrix = []
        for i in range(20, len(fit_data)):  # Start from 20 to have enough for rolling stats
    ...
    def detect_regime(self, data: pd.DataFrame) -> RegimeDetectionResult:
        ...
        # HMM prediction
        hmm_state = self.hmm_model.predict(current_features)[0]
        hmm_probabilities = self.hmm_model.predict_proba(current_features)[0]
