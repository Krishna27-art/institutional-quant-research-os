...
class ORBConfig:
    ...
    atr_period: int = 14
    atr_stop_multiplier: float = 1.0  # 100% of ATR (increased from 10% for NIFTY futures)
...
class ORBBacktesterZarattini:
    ...
    def _check_long_breakout(
        ...
        for row in post_orb_data.itertuples():
            idx = row.Index
            if getattr(row, 'high') > orb_high:
                # Stop order fills at the known breakout level; same-bar high is
                # not known when the order triggers.
    ...
    def _check_short_breakout(
        ...
        for row in post_orb_data.itertuples():
            idx = row.Index
            if getattr(row, 'low') < orb_low:
                # Stop order fills at the known breakdown level; same-bar low is
                # not known when the order triggers.
                entry_price = orb_low
                stop_loss = entry_price + (atr * self.config.atr_stop_multiplier)
                target = entry_price - (atr * self.config.atr_stop_multiplier * self.config.target_profit_multiplier)
    ...
    def _execute_trade(
        ...
        for row in day_data[day_data.index > entry_time].itertuples():
            idx = row.Index
            if side == 'LONG':
                if getattr(row, 'low') <= stop_loss:
                    exit_price = stop_loss
                    exit_reason = "stop_loss"
                    break
                elif getattr(row, 'high') >= target:
                    exit_price = target
                    exit_reason = "target"
                    break
            else:  # SHORT
                if getattr(row, 'high') >= stop_loss:
                    exit_price = stop_loss
                    exit_reason = "stop_loss"
                    break
                elif getattr(row, 'low') <= target:
                    exit_price = target
                    exit_reason = "target"
                    break
        else:
...
def scan_symbols(data: dict, current_time) -> List[dict]:
    ...
    for symbol, df in data.items():
        ...
        if is_daily or len(day_data) < config.orb_minutes + 1:
            # Not enough intraday data for ORB. Skip symbol.
            continue
