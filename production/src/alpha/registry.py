...
class AlphaRegistry:
    ...
    def demote_alpha(self, alpha_id: str, reason: str, metadata: Optional[Dict[str, Any]] = None):
        """Demote an alpha to rejected/dead and record its death."""
        alpha = self.get(alpha_id)
        if alpha:
            alpha.status = AlphaStatus.REJECTED
            from .lifecycle_manager import LifecycleManager, AlphaState
            lm = LifecycleManager(registry_dir="production/alpha_registry")
            full_metadata = {
                'created': alpha.created_at.isoformat(),
                'author': 'system',
                'hypothesis': alpha.logic,
                'death_reason': reason
            }
            if metadata:
                full_metadata.update(metadata)
            lm.transition_alpha(alpha_id, AlphaState.DEAD, full_metadata)
