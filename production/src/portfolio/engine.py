...
class PortfolioAllocator:
    ...
    def __init__(
        self,
        total_capital: float = TRADING_CAPITAL,
    ...
    def hierarchical_risk_parity(self, covariance: pd.DataFrame) -> Dict[str, float]:
        ...
        def get_quasi_diag(link):
            ...
            def expand(node_id: int) -> list[int]:
                if node_id < n_assets:
                    return [node_id]
                left, right = link[node_id - n_assets, :2]
                return expand(int(left)) + expand(int(right))

            return expand(int(2 * n_assets - 2))
    ...
    def allocate_from_alpha_signals(
        ...
        for symbol, score, direction, signal in sorted(scored, key=lambda item: item[1], reverse=True):
            ...
            allocations.append(PortfolioAllocation(symbol=symbol, weight=signed_weight, capital=allocated, score=score * direction))
        ...
        **kwargs
        ...
        if method == "hrp":
            if price_history is not None:
                returns = price_history.pct_change(fill_method=None).dropna()
        ...
        sector_exposure = {}
        for symbol, score, signal in sorted(scored, key=lambda item: item[1], reverse=True):
            ...
            allocated *= vol_multiplier
            direction = float(self._signal_value(signal, "direction", 1.0) or 1.0)
            dir_val = 1.0 if direction >= 0 else -1.0
            allocations.append(PortfolioAllocation(symbol=symbol, weight=weight * dir_val, capital=allocated, score=score * direction))
        return allocations
