...
from src.execution.adapters.backtest_adapter import BacktestAdapter, BacktestConfig, BacktestResult
from src.execution.adapters.paper_adapter import PaperAdapter, PaperConfig, PaperResult
from src.execution.adapters.live_adapter import LiveAdapter, LiveConfig, LiveResult
from src.execution.cost_models import MarketImpactModel, TransactionCostModel
from src.execution.risk_checks import PreTradeRiskChecker, PostTradeRiskChecker
    ...
    def process_signal(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.portfolio_allocator:
            try:
                ...
                if symbol == 'NIFTY':
                    allocated_qty = max(25, (allocated_qty // 25) * 25)
                elif symbol == 'BANKNIFTY':
                    allocated_qty = max(15, (allocated_qty // 15) * 15)
                elif allocated_qty == 0:
                    allocated_qty = 1  # Fallback to 1 for un-lotted symbols if strictly needed, or just let it fail.
