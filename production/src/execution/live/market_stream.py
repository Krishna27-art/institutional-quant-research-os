...
class NSEWebSocketStream:
    ...
    async def connect(self):
        ...
        while self.reconnect_attempts < self.max_reconnect_attempts:
            try:
                ...
                self.last_message_time = datetime.now(ZoneInfo("Asia/Kolkata"))
    ...
    def is_alive(self) -> bool:
        ...
        if self.last_message_time:
            time_since_last = (datetime.now(ZoneInfo("Asia/Kolkata")) - self.last_message_time).total_seconds()
    ...
    def _parse_timestamp(self, raw_ts):
        ...
        if isinstance(raw_ts, datetime):
            if raw_ts.tzinfo is None:
                return raw_ts.replace(tzinfo=tz)
            return raw_ts.astimezone(tz)
