...
class InstitutionalRiskEngine:
    ...
    def check_pre_trade_limits_rust(self, order_price: float, order_quantity: int, symbol: str) -> bool:
        import ctypes
        import sys
        import platform
        from pathlib import Path
        ...
        base_dir = Path(__file__).parent / "rust_risk_engine" / "target" / "release"
        system = platform.system()
        if system == "Darwin":
            lib_path = base_dir / "librust_risk_engine.dylib"
        elif system == "Windows":
            lib_path = base_dir / "rust_risk_engine.dll"
        else:
            lib_path = base_dir / "librust_risk_engine.so"
            
        if not lib_path.exists():
            logger.error(f"Rust risk engine library not found at {lib_path}")
            return True # Fallback
            
        try:
            class RiskLimits(ctypes.Structure):
                _fields_ = [
                    ("max_position_value", ctypes.c_double),
                    ("max_daily_orders", ctypes.c_int),
                    ("max_daily_trades", ctypes.c_int),
                    ("max_exposure", ctypes.c_double),
                    ("max_leverage", ctypes.c_double),
                ]
                
            class OrderContext(ctypes.Structure):
                _fields_ = [
                    ("current_position_value", ctypes.c_double),
                    ("current_exposure", ctypes.c_double),
                    ("daily_orders_count", ctypes.c_int),
                    ("daily_trades_count", ctypes.c_int),
                    ("order_price", ctypes.c_double),
                    ("order_quantity", ctypes.c_int),
                ]
                
            lib = ctypes.CDLL(str(lib_path))
            lib.check_order_limits.argtypes = [ctypes.POINTER(RiskLimits), ctypes.POINTER(OrderContext), ctypes.c_char_p]
            lib.check_order_limits.restype = ctypes.c_bool
            
            limits = RiskLimits(self.capital * 0.1, 100, 100, self.capital * 0.2, self.max_leverage)
            ctx = OrderContext(0.0, 0.0, 0, 0, float(order_price), int(order_quantity))
            
            return lib.check_order_limits(ctypes.byref(limits), ctypes.byref(ctx), symbol.encode('utf-8'))
        except Exception as e:
            logger.error(f"Error calling Rust FFI: {e}")
            return True
