...
class HMMDetector:
    ...
    def predict_regime(self, df: pd.DataFrame) -> pd.Series:
        if self.model is None:
            # Rule-based fallback
            features = self._feature_frame(df)
            vol = features['vol'].fillna(0.0).values
            ret_5d = features['ret'].rolling(5).sum().fillna(0.0).values
            conditions = [vol >= 0.25, ret_5d >= 0.01, ret_5d <= -0.01]
            choices = ['high_vol', 'bull', 'bear']
