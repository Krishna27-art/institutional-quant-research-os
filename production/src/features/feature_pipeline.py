...
class FeaturePipeline:
    ...
    _feature_cache = {}  # (symbol, last_ts) -> feature_dict
    ...
    def compute_features(
        ...
        key = (symbol, ts) if symbol else None
        
        if key and key in self._feature_cache:
        ...
        # P1-2 Fix: Replace options mock 0.0 with deterministic proxies
        iv_proxy = features["realized_volatility_20d"] * 1.2
        iv = float((options_data or {}).get("iv") or iv_proxy)
        iv_percentile = float((options_data or {}).get("iv_percentile") or 50.0)
        ...
        pcr_proxy = 1.0 if close.iloc[-1] > close.tail(5).mean() else 0.8
        features["pcr"] = float((options_data or {}).get("pcr") or pcr_proxy)
        
        # FII flow is hard to proxy accurately, use volume-price proxy
        fii_proxy = (close.iloc[-1] - open_.iloc[-1]) * volume.iloc[-1] / 1e7
        features["fii_dii_flow"] = float((flow_data or {}).get("fii_dii_flow", (flow_data or {}).get("net_flow")) or fii_proxy)
        ...
        final_features = self._clip_and_order(features)
        if key:
            self._feature_cache[key] = final_features
    ...
    def _atr(self, high: pd.Series, low: pd.Series, close: pd.Series, period: int) -> pd.Series:
        ...
        tr = pd.Series(np.maximum(tr1, np.maximum(tr2, tr3)), index=high.index)
