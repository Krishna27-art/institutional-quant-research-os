...
try:
    ...
    logger.warning("Market calendar not available - using basic validation")
...
class DataQualityEngine:
    ...
    def __init__(self, config: DataQualityConfig = None):
        ...
        if MARKET_CALENDAR_AVAILABLE:
            ...
            logger.info("DataQualityEngine initialized with market calendar support")
        else:
            self.market_calendar = None
            logger.info("DataQualityEngine initialized (basic mode - no market calendar)")
    ...
    def is_trading_hour(self, dt: datetime) -> bool:
        ...
        if self.market_calendar:
            return self.market_calendar.is_trading_time(dt)
    ...
    def is_stale(self, symbol: str, data_type: str = '1min') -> bool:
        ...
        if not self.is_trading_hour(now):
            # Outside trading hours, allow longer staleness
            threshold = self.config.stock_staleness_threshold * 6  # 6x tolerance outside hours
        else:
            # During trading hours, use strict threshold
            threshold = self.config.stock_staleness_threshold
        
        staleness_seconds = (now - last_update).total_seconds()
