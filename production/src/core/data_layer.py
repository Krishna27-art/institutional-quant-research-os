...
Enhanced with data validation pipeline integration for institutional-grade
...
try:
    ...
    logger.warning("Data validation pipeline not available - validation disabled")
...
class YahooFeed(DataFeed):
    ...
    async def get_historical(
        ...
        try:
            from src.data.truth import get_price_history, NSE_UNIVERSE
...
class DataManager:
    ...
    def __init__(self, config: dict):
        ...
        if VALIDATION_PIPELINE_AVAILABLE:
            self._validation_pipeline = get_validation_pipeline()
            logger.info("Data validation pipeline enabled")
        else:
            logger.warning("Data validation pipeline not available")
    ...
    async def get_data(
        ...
        """
        Get historical data with Arctic caching and validation.

        Flow: Check Arctic → Check Redis → Fetch from API → Validate → Cache
        ...
        Args:
            ...
            skip_validation: Whether to skip validation (for debugging only)
        ...
        if use_cache and self._arctic is not None:
            ...
            if not arctic_data.empty:
                ...
                if not skip_validation and self._validation_pipeline:
                    validated = self._validation_pipeline.validate(
                        symbol, arctic_data, DataSourceType.ARCTIC,
                        {'data_type': interval}
                    )
                    if not validated.metadata.get('is_valid', False):
                        logger.warning(f"Cached data failed validation for {symbol}, refetching")
                    else:
        ...
        # 3. Validate data
        if not skip_validation and self._validation_pipeline:
            source_type = DataSourceType.ZERODHA if feed_name == "zerodha" else DataSourceType.YAHOO
            validated = self._validation_pipeline.validate(
                symbol, data, source_type, {'data_type': interval}
            )
            
            if not validated.metadata.get('is_valid', False):
                logger.error(f"Data validation failed for {symbol} from {source_type.value}")
                # Return empty DataFrame to prevent using invalid data
                return pd.DataFrame()
