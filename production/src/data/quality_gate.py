...
3. No stale prices (>15 identical closes = stale feed)
...
class DataQualityGate:
    ...
    def validate(
        ...
        # ── Reindexing and forward-fill to maintain timegrid ────────
        if self.drop_bad_rows and rows_out < rows_in and rows_out > 0:
...
# ── Backwards compatibility classes, enums, and methods ──────────────────
...
def check_data_quality(self, symbol: str, data: pd.DataFrame, data_source: str = "unknown") -> DataQualityCheck:
    """Exposes consolidated validate checks in format expected by DataQualityEngine."""
    clean_df, result = self.validate(symbol, data)
    
    # Map violations/passed status to DataQualityStatus
    ...
    elif any("stale" in v for v in result.violations):
        status = DataQualityStatus.STALE
    elif any("inconsistency" in v or "ohlc" in v for v in result.violations):
        status = DataQualityStatus.CORRUPT
    elif not result.passed:
        status = DataQualityStatus.INCOMPLETE
        
    last_update = data.index[-1].to_pydatetime() if not data.empty and isinstance(data.index, pd.DatetimeIndex) else datetime.now(timezone.utc)
    staleness = (datetime.now(timezone.utc) - last_update).total_seconds()
    
    # Track check in history
    self._check_history.append((symbol, status))
    if len(self._check_history) > 1000:
        self._check_history = self._check_history[-1000:]
        
    if status != DataQualityStatus.GOOD:
        self.blocked_symbols[symbol] = (status, ", ".join(issues))
    ...
    return DataQualityCheck(
        ...
        is_acceptable=result.passed,
        issues=issues,
        recommendations=["Refresh data from source" if status != DataQualityStatus.GOOD else "No action required"]
    )


def get_blocked_symbols(self) -> Dict[str, Tuple[DataQualityStatus, str]]:
    """Returns blocked symbols and reasons."""
    return self.blocked_symbols.copy()


def get_quality_summary(self) -> Dict[str, int]:
    """Returns summary counts of all checks."""
    total = len(self._check_history)
    good = sum(1 for _, s in self._check_history if s == DataQualityStatus.GOOD)
    stale = sum(1 for _, s in self._check_history if s == DataQualityStatus.STALE)
    incomplete = sum(1 for _, s in self._check_history if s == DataQualityStatus.INCOMPLETE)
    corrupt = sum(1 for _, s in self._check_history if s == DataQualityStatus.CORRUPT)
    missing = sum(1 for _, s in self._check_history if s == DataQualityStatus.MISSING)
...
DataQualityGate.check_data_quality = check_data_quality
DataQualityGate.get_blocked_symbols = get_blocked_symbols
DataQualityGate.get_quality_summary = get_quality_summary

# Compatibility aliases
DataQualityEngine = DataQualityGate
get_data_quality_engine = get_quality_gate
