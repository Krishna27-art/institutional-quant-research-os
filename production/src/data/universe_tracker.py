...
# Single source of truth for NSE Holidays (2024-2027)
NSE_HOLIDAYS = {
    # 2024
...
class NSEMarketCalendar:
    ...
    def _load_holidays(self) -> Dict[date, Holiday]:
        """Load NSE holidays from single source of truth"""
        holidays = {}
        for dt in NSE_HOLIDAYS:
            holidays[dt] = Holiday(date=dt, name="Holiday")
        return holidays
    ...
    def _is_muhurat(self, dt: datetime) -> bool:
        ...
        ist_dt = dt.astimezone(ZoneInfo("Asia/Kolkata")) if dt.tzinfo else dt.replace(tzinfo=ZoneInfo("Asia/Kolkata"))
        dt_date = ist_dt.date()
        if dt_date in self.muhurat_dates:
            start, end = self.muhurat_dates[dt_date]
            return start <= ist_dt.time() <= end
        return False
    ...
    def is_trading_session(self, dt: datetime) -> bool:
        ...
        return regular.start_time <= ist_dt.time() <= regular.end_time
    ...
    def get_session(self, dt: datetime) -> Optional[str]:
        ...
        for session_name, session in self.sessions.items():
            if session.start_time <= ist_dt.time() <= session.end_time:
                return session_name
        return None
    ...
    def time_until_market_close(self, dt: datetime) -> timedelta:
        ...
        ist_dt = dt.astimezone(ZoneInfo("Asia/Kolkata")) if dt.tzinfo else dt.replace(tzinfo=ZoneInfo("Asia/Kolkata"))
        regular = self.sessions['regular']
        close_time = datetime.combine(ist_dt.date(), regular.end_time, tzinfo=ZoneInfo("Asia/Kolkata"))
        return close_time - ist_dt
    ...
    def time_until_market_open(self, dt: datetime) -> timedelta:
        ...
        from zoneinfo import ZoneInfo
        ist_dt = dt.astimezone(ZoneInfo("Asia/Kolkata")) if dt.tzinfo else dt.replace(tzinfo=ZoneInfo("Asia/Kolkata"))
        next_day = self.next_trading_day(ist_dt.date())
        open_time = datetime.combine(next_day, self.sessions['regular'].start_time, tzinfo=ZoneInfo("Asia/Kolkata"))
        return open_time - ist_dt
