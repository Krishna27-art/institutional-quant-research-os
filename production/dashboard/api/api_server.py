...
from src.core.market_calendar import market_status as get_market_status_func
from src.data.nifty50_symbols import get_nifty50_symbols
from src.alpha.manager import AlphaManager
from src.alpha.prediction_storage import PredictionStorage
from src.data.quality_gate import get_quality_gate, DataQualityGate
...
data_quality_gate = get_quality_gate()
...
risk_engine = InstitutionalRiskEngine(capital=TRADING_CAPITAL)
...
def calculate_real_risk(trade_metrics) -> dict:
    ...
    try:
        ...
        if risk_positions and not hist_df.empty:
            ...
            risk_engine.capital = TRADING_CAPITAL + total_pnl_val
    ...
    # Return 0.0 values if calculation fails or there are no positions
    return {
        'var': 0.0,
        'cvar': 0.0,
        'tail_risk': 0.0
...
# In local development/testing, default to a placeholder if .env is incomplete.
if not SECRET_KEY:
...
async def get_state():
    ...
    try:
        ...
        async with state_lock:
            publisher.state['nav'] = sanitize_float(TRADING_CAPITAL + total_pnl_val, TRADING_CAPITAL)
...
async def get_indices():
    ...
    try:
        ...
        if nifty_data.empty or banknifty_data.empty:
            return {"error": "data unavailable", "indices": []}
        ...
        for name, data in indices_data.items():
            if not data.empty:
                try:
                    clean_df, check = data_quality_gate.validate(name.replace(" ", "_"), data)
                    quality_results[name] = {
                        "status": "PASS" if check.passed else "FAIL",
                        "is_acceptable": check.passed,
                        "staleness_seconds": 0
                    }
                except Exception:
        ...
        nifty_change = ((nifty_close - nifty_prev) / nifty_prev) * 100 if (len(nifty_data) > 1 and nifty_prev != 0.0) else 0.0
        ...
        banknifty_change = ((banknifty_close - banknifty_prev) / banknifty_prev) * 100 if (len(banknifty_data) > 1 and banknifty_prev != 0.0) else 0.0
        
        finnifty_close = finnifty_data['Close'].iloc[-1] if not finnifty_data.empty else 0.0
        finnifty_prev = finnifty_data['Close'].iloc[-2] if len(finnifty_data) > 1 else finnifty_close
        finnifty_change = ((finnifty_close - finnifty_prev) / finnifty_prev) * 100 if (len(finnifty_data) > 1 and finnifty_prev != 0.0) else 0.0
        
        vix_close = vix_data['Close'].iloc[-1] if not vix_data.empty else 0.0
        vix_prev = vix_data['Close'].iloc[-2] if len(vix_data) > 1 else vix_close
        vix_change = ((vix_close - vix_prev) / vix_prev) * 100 if (len(vix_data) > 1 and vix_prev != 0.0) else 0.0
        
        result = [
            {"name": "NIFTY 50", "value": round(sanitize_float(nifty_close, 0.0), 2), "change": round(sanitize_float(nifty_change, 0.0), 2), "data_quality": quality_results.get("NIFTY 50", {})},
            {"name": "BANKNIFTY", "value": round(sanitize_float(banknifty_close, 0.0), 2), "change": round(sanitize_float(banknifty_change, 0.0), 2), "data_quality": quality_results.get("BANKNIFTY", {})},
            {"name": "FINNIFTY", "value": round(sanitize_float(finnifty_close, 0.0), 2), "change": round(sanitize_float(finnifty_change, 0.0), 2), "data_quality": quality_results.get("FINNIFTY", {})},
            {"name": "India VIX", "value": round(sanitize_float(vix_close, 0.0), 2), "change": round(sanitize_float(vix_change, 0.0), 2), "data_quality": quality_results.get("India VIX", {})}
        ]
        return result
    except Exception as e:
...
async def get_market_status():
    ...
    try:
        ...
        return {
            "is_open": False,
            "is_pre_open": False,
            "current_time": datetime.now(timezone.utc).strftime("%H:%M:%S IST"),
...
async def get_health():
    ...
    return {
        "latest_data_timestamp": datetime.now(timezone.utc).isoformat(),
...
async def get_system_health():
    ...
    try:
        ...
        # Data Quality Engine
        try:
            dq_summary = {"status": "ok", "passed": True} # Default summary since the gate drops bad data
            dq_available = True
        except:
            dq_summary = {}
            dq_available = False
        ...
        return {
            ...
            "data_quality": {
                ...
                "engine": "src/data/quality_gate.py",
                "summary": dq_summary
            },
            "trade_logger": {
            ...
            "data_sources": {
                ...
                "indices": "REAL (truth.py)",
                "data_quality": "REAL (src/data/quality_gate.py)",
...
async def get_models():
    ...
    try:
        ...
        return {
            ...
            "models": models,
            "last_updated": model_summary.get("last_updated", datetime.now(timezone.utc).isoformat())
        }
    except Exception as e:
        return {
            "total_models": 0,
...
async def get_stock_profile(symbol: str):
    ...
    from src.data.truth import get_latest_prices
    try:
...
async def get_strategies():
    ...
    try:
        ...
        return ["orb", "momentum", "mean_reversion", "options_carry", "daily_momentum"]
...
async def get_risk_portfolio():
    ...
    try:
        ...
        risk_data, _ = await asyncio.to_thread(calculate_real_risk, metrics)
        return {
            "var_99": risk_data["var"],
            "cvar_95": risk_data["cvar"],
            "tail_risk": risk_data["tail_risk"],
...
async def run_startup_initialization():
    ...
    try:
        ...
        from src.data.truth import refresh_prices
...
async def update_market_data():
    ...
    while True:
        try:
            ...
            try:
                indices_data = await get_indices()
                async with state_lock:
            ...
            # Update signals from real PredictionStorage database
            try:
                preds = await asyncio.to_thread(prediction_storage.get_predictions, limit=15)
                signal_dicts = []
                for p in preds:
                    signal_dicts.append({
                        ...
                        "symbol": p.symbol,
                        "strategy": p.strategy,
                        "direction": 1 if p.direction.lower() == "long" else -1,
                        "confidence": sanitize_float(p.confidence, 0.5)
                    })
                async with state_lock:
                    publisher.state['signals'] = signal_dicts
            except Exception as sig_err:
                logger.warning(f"Error updating live signal stream from storage: {sig_err}")
                async with state_lock:
                    publisher.state['signals'] = []
        ...
        # Removed artificial 2-second delay per UNIFIED FIX PLAN
        ...
        try:
            ...
            if pending:
                ...
                for pred in pending:
                    try:
                        symbol = pred.symbol
                        age_days = max(1, (datetime.now(timezone.utc) - pred.timestamp).days)
