...
def yf_download_with_fallback(tickers, max_retries=3, initial_delay=2.0, **kwargs):
    """Downloads prices strictly. Fallbacks to other symbols are dangerous and have been removed."""
    raw = yf_download_with_retry(tickers, max_retries, initial_delay, **kwargs)
    
    if raw.empty:
        raise ValueError(f"Failed to fetch real data for {tickers} after {max_retries} retries. No fallback is allowed.")
...
# Fallback tickers removed to prevent silent data corruption (bug #5)
...
def verify_prices(is_live: bool = False) -> dict:
    """
    ...
    try:
        for symbol in fallback_ranges.keys():
            ...
            price_date = pd.to_datetime(row["date"].iloc[0]).date()
            ...
            if is_live and ok:
                ...
                if price_date > today:
                    ok = False
                    expected_range_str = f"future_date_failed: price date {price_date} is after today {today}"
