...
def test_data_quality_gate_rule2_ohlc_inconsistency():
    ...
    dates = pd.date_range(datetime.now(timezone.utc), periods=5, freq="D")
    df = pd.DataFrame(
        {
...
def test_data_quality_gate_rule3_stale_prices():
    ...
    assert not result.passed
    assert len(clean_df) == 5
    assert clean_df.loc[dates[2], "volume"] == 0.0
    assert clean_df.loc[dates[3], "volume"] == 0.0
    assert clean_df.loc[dates[2], "close"] == 100.0
...
def test_data_quality_gate_rule5_overnight_gap_soft():
    ...
    dates = pd.date_range(datetime.now(timezone.utc), periods=3, freq="D")
    df = pd.DataFrame(
        {
...
def test_prediction_registry_basic(tmp_path):
    ...
    # Resolve prediction
    realized_ret = registry.resolve_prediction(pred_id, exit_price=3090.0, exit_timestamp=datetime.now(timezone.utc))
