...
def deflated_sharpe(
    ...
    Adjusts the observed Sharpe ratio for multiple testing bias using
    the probability of observing a Sharpe ratio as extreme under
    ...
    where:
    E[SR] ≈ (1 + 0.5*SR^2 - skew*SR + (kurt-3)/4*SR^2) / n_trials
    std(SR) ≈ sqrt(E[SR])
    ...
    Args:
        ...
        kurt: Kurtosis of returns (excess kurtosis, normal = 0)
    ...
    # Calculate expected Sharpe under null
    ...
    # Z-score for multiple testing
    z = stats.norm.ppf(1 - 1 / n_trials)
    
    # Deflated Sharpe
    deflated_sr = observed_sharpe - sr_benchmark - std_sr * z
...
def probability_of_failure(
    ...
    """
    Calculate probability of failure (Type I error).
    ...
    Returns:
        Probability of observing such Sharpe by chance
    """
    # Calculate deflated Sharpe
    deflated_sr = deflated_sharpe(observed_sharpe, n_trials, skew, kurt, sr_benchmark)
...
class AdvancedValidator:
    ...
    def validate_backtest(
        ...
        metrics['kurtosis'] = stats.kurtosis(returns.values)
