...
class TestHonestEvaluation:
    ...
    def test_deflated_sharpe(self):
        ...
        try:
            ...
            sharpe = 1.5
            deflated = eval_module.deflated_sharpe_ratio(
                returns=returns,
                sharpe_ratio=sharpe,
                num_trials=100
            )
            ...
            assert deflated >= 0
            # Deflated Sharpe should be <= original Sharpe
            assert deflated <= sharpe + 0.1  # Allow small tolerance
            
        except ImportError:
            pytest.skip("honest_evaluation not available")
    
    def test_minimum_track_record(self):
        """Test minimum track record length calculation."""
        try:
            from foundation.honest_evaluation import HonestEvaluation
            
            eval_module = HonestEvaluation()
            sharpe = 1.5
            min_years = eval_module.minimum_track_record_length(sharpe)
            
            assert min_years > 0
            # Higher Sharpe should require shorter track record
            assert min_years < 20  # Should be reasonable
