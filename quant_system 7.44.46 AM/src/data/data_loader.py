    ...
    def adjust(df: pd.DataFrame, actions_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, action in actions.iterrows():
            ...
            if action_type == "split" and val > 0:
                ...
                post_action_df = df[df.index >= action_date]
                if not post_action_df.empty:
                    base_price = post_action_df["close"].iloc[0]
                    if base_price > 0:
...
class NSEDataLoader:
    ...
    def get_historical_bars(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            from data.truth import get_corporate_actions
    ...
    def _get_yahoo_finance_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        Query the trusted daily prices from market_truth.db instead of downloading directly.
        """
        from data.truth import get_price_history, refresh_prices
