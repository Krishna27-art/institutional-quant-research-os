...
class InstitutionalFeatureFactory:
    ...
    def _compute_hurst(self, returns: pd.Series, window: int) -> pd.Series:
        """Compute Hurst exponent using trailing log-log variance scaling."""
        hurst = pd.Series(index=returns.index, dtype=float)
        
        for i in range(window, len(returns)):
            subset = returns.iloc[i-window:i].dropna()
            hurst.iloc[i] = self._hurst_estimate(subset)
        
        return hurst
    
    def _compute_long_memory_d(self, returns: pd.Series, window: int) -> pd.Series:
        """Compute fractional long-memory parameter from Hurst estimate."""
        d = pd.Series(index=returns.index, dtype=float)
        
        for i in range(window, len(returns)):
            subset = returns.iloc[i-window:i].dropna()
            hurst = self._hurst_estimate(subset)
            d.iloc[i] = np.clip(hurst - 0.5, -0.5, 0.5) if np.isfinite(hurst) else np.nan
        
        return d

    def _hurst_estimate(self, returns: pd.Series, max_lag: int = 20) -> float:
        """Estimate H where std(r[t+lag] - r[t]) scales as lag ** H."""
        ...
        for lag in lags:
            ...
            scale = np.std(diff)
            tau.append(scale if np.isfinite(scale) and scale > 0 else np.nan)
