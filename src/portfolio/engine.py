    ...
    def black_litterman(
        ...
        for i, view in enumerate(views):
            for asset_name, weight in zip(view.assets, view.weights):
                try:
                    asset_idx = int(asset_name) if asset_name.isdigit() else 0
                except ValueError:
                    asset_idx = 0
        ...
        view_variances = np.diag(P @ covariance_matrix @ P.T)
        for i, view in enumerate(views):
...
class PortfolioAllocator:
    ...
    def hierarchical_risk_parity(self, covariance: pd.DataFrame) -> Dict[str, float]:
        ...
        def get_quasi_diag(link):
            ...
            while sort_ix.max() >= len(dist):
                ...
                df0 = link[df0[0].values - len(dist), :2]
                df0 = pd.DataFrame(df0, index=df0.index)
    ...
    def black_litterman(
        ...
        return self.black_litterman_model.black_litterman(prior_returns, covariance, views)
