...
# Module previously crashed on startup if hmmlearn was not installed.
# Now falls back to rule-based regime detection gracefully.
try:
    ...
    logging.getLogger(__name__).warning(
        "hmmlearn not installed — HMM training disabled. "
...
class HMMRegimeEngine:
    ...
    def predict_regime(self, features: dict[str, float], timestamp: datetime) -> RegimeState:
        ...
        # Institutional regime filters from market breadth and average correlation
        pct_above_20 = clean.get("pct_above_20ema", 0.5)
        avg_corr = clean.get("avg_correlation", 0.0)

        if max(realized_vol, implied_vol) >= self.config.high_vol_threshold or vix >= 25 or (avg_corr > 0.5 and realized_vol > 0.20):
            regime = Regime.HIGH_VOL
            confidence = 0.65
        elif ret_5d >= self.config.bull_return_threshold or pct_above_20 > 0.75:
            regime = Regime.BULL_TREND
            confidence = min(0.9, 0.55 + abs(ret_5d) * 5)
        elif ret_5d <= self.config.bear_return_threshold or pct_above_20 < 0.25:
    ...
    def get_position_size_multiplier(self) -> float:
        ...
        # Retrieve VIX or implied volatility to scale down position sizes during turbulence
        vix = latest.get("india_vix", latest.get("implied_vol", 0.15) * 100)
        if vix > 30:
            return 0.3
        elif vix > 20:
            return 0.6
...
class RobustHMMRegime:
    ...
    def _feature_frame(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add market breadth and correlation features if they are in the input DataFrame
        for col in ['pct_above_20ema', 'pct_above_50ema', 'pct_above_200ema', 'new_highs_20d', 'new_lows_20d', 'avg_correlation']:
    ...
    def _transform_features(self, X: np.ndarray, is_fit: bool = False) -> np.ndarray:
        """Apply feature scaling, correlation-based drops, and PCA."""
        if is_fit:
            # Scale features
            ...
            # Apply PCA to retain 95% variance if features > 5
    ...
    def fit(self, df: pd.DataFrame):
        ...
        if len(X) < 100:
            logger.warning(f"Need at least 100 observations, got {len(X)}. Falling back to rule-based regime classifier.")
            self.model = None
            self.n_states = 0
            self.state_labels = {}
            return self
        ...
        if not HMM_AVAILABLE or hmm is None:
            logger.error("hmmlearn not available — cannot fit HMM. Install with: pip install hmmlearn>=0.2.8")
    ...
    def predict_regime(self, df: pd.DataFrame) -> pd.Series:
        ...
        if self.model is None:
            ...
            for idx, row in features.iterrows():
                vol = row.get('vol', 0.0)
                # Compute 5-day return from returns
