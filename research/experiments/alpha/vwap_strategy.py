...
class VWAPStrategy:
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame, vvg_label: str):
        ...
        session_data = df[df.index.normalize() == df.index[-1].normalize()] if isinstance(df.index, pd.DatetimeIndex) else df
        pv = session_data["open"] * session_data["volume"]
        vwap = float(pv.cumsum().iloc[-1] / max(session_data["volume"].cumsum().iloc[-1], 1))
