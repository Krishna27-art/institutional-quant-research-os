...
class VWAPTrendBacktesterZarattini:
    ...
    def calculate_vwap(self, data: pd.DataFrame) -> pd.Series:
        """
        Calculate point-in-time VWAP (Volume-Weighted Average Price).
        
        VWAP = Σ(Price × Volume) / Σ(Volume)

        Uses each bar's open, which is known when a live decision is made. The
        high/low/close version is useful for reporting after a bar closes, but
        it is not a valid live entry feature.
        """
        session = pd.Series(data.index.normalize(), index=data.index)
        pv = data['open'] * data['volume']
        cumulative_pv = pv.groupby(session).cumsum()
        cumulative_volume = data['volume'].groupby(session).cumsum()
    ...
    def calculate_vwap_std(self, data: pd.DataFrame, vwap: pd.Series, window: int = 20) -> pd.Series:
        """
        Calculate prior-bar rolling sigma of price minus VWAP.

        The signal uses sigma known before the current bar. This prevents the
        current close from influencing its own normalization.
        """
        distance = (data['open'] - vwap) / vwap
        return distance.groupby(data.index.normalize()).transform(
            lambda s: s.shift(1).rolling(window=window, min_periods=max(5, window // 2)).std()
        )
    ...
    def run_backtest(
        ...
        vwap_distance_sigma = ((data_filtered['open'] - vwap) / vwap) / vwap_std
        vwap_distance_sigma = vwap_distance_sigma.replace([np.inf, -np.inf], np.nan)
    ...
    def _process_day(
        ...
        for idx, row in day_data.iterrows():
            ...
            if pd.isna(current_vwap) or pd.isna(current_std) or pd.isna(current_distance):
                continue
            ...
            # Check for exit conditions
                ...
                if current_position == 'LONG':
                    ...
                    # Target: price > VWAP by 3σ (only if minimum holding time met)
                    elif current_distance > self.config.target_profit_sigma:
                        if min_holding_met:  # CRITICAL FIX: Enforce minimum holding time for target too
                            exit_triggered = True
                            exit_reason = "target"
                ...
                if exit_triggered:
                    self._execute_trade(
                        ...
                        exit_time=idx,
                        entry_price=day_data.loc[entry_bar, 'open'],
    ...
    def _execute_trade(
        ...
        if len(self.trades) < 50:
            print(f"VWAP TRADE #{len(self.trades)+1}: {side}")
            print(f"  Entry: {actual_entry:.2f}, VWAP_entry: {vwap_entry:.2f}, Distance: {vwap_distance_sigma:.2f}σ")
            print(f"  Exit: {actual_exit:.2f}, VWAP_exit: {vwap_exit:.2f}")
            print(f"  PnL: ₹{net_pnl:,.2f}, Reason: {exit_reason}, Holding: {holding_minutes:.1f}min")
            print(f"  Check: LONG should have price > VWAP, SHORT should have price < VWAP")
            if side == 'LONG':
                print(f"  Entry check: {actual_entry > vwap_entry}, Exit check: {actual_exit > vwap_exit}")
            else:
                print(f"  Entry check: {actual_entry < vwap_entry}, Exit check: {actual_exit < vwap_exit}")
